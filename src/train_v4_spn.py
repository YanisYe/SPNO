...
from sp_operator_v4 import ClimaX

from datamodule import GlobalForecastDataModule as datamodule2

# /home/jjh/new/downscaling/Data_generation/datamodule.py
# from Encoder.encoder import Encoder
from lr_scheduler import LinearWarmupCosineAnnealingLR

# from latent_foundation.dynamo_ssl_main.models.ssl.dynamo import DynaMoSSL 
from metrics import (
    lat_weighted_acc,
    lat_weighted_mse,
...
from spherical_encoder_v3 import WeatherSphericalEncoder
...
model_name = "SphereConv_Physics_Downscaling_Spherical_Operator"
...
num_epochs = 500 
stride_ = 2
# logging_folder 将在 main() 函数中定义，添加时间戳
H, W = 32, 64
...
# low_data1 = datamodule1(buffer_size=100,root_dir=r'/home/hunter/workspace/climate/mnt/',variables=default_vars,out_variables=out_vars,batch_size=batch_size,predict_range=pred_range)
# high_data1 = datamodule1(buffer_size=100,root_dir=r'/home/hunter/workspace/climate/mnt/2.8125deg_npz/',variables=out_vars,out_variables=out_vars,batch_size=batch_size,predict_range=pred_range)
low_data = datamodule2(buffer_size=100,root_dir=r'/home/hunter/workspace/climate/mnt/',variables=default_vars,out_variables=out_vars,batch_size=batch_size,predict_range=pred_range, num_workers=1)
high_data = datamodule2(buffer_size=100,root_dir=r'/home/hunter/workspace/climate/mnt/2.8125deg_npz/',variables=out_vars,out_variables=out_vars,batch_size=batch_size,predict_range=pred_range, num_workers=1)
...
# x1,_,lead_times1, variables1,_ = next(iter(low_train_dataloader1))
# y1,_,_, _, out_variables1 = next(iter(high_train_dataloader1))
# x2,_,lead_times2, variables2,_ = next(iter(low_train_dataloader2))
# y2,_,_, _, out_variables2 = next(iter(high_train_dataloader2))

num_scales = 4

def train(encoder, model: ClimaX, epoch, high_dataloader, low_dataloader, coords, optimizer, scheduler, device, accumulation_steps=6, global_step=0, physics_loss=None, physics_weight=0.0, physics_loss_normalize=False):
    model.train()
...
# 测试/验证循环
def evaluate(encoder, model:ClimaX, low_dataloader, high_dataloader, coords, device, split_name="eval"):
    model.eval()
...
def main():
    ...
    physics_weight_strategy = "linear"  # 调度策略: "linear", "step", "cosine", "adaptive"
    ...
    if use_physics_loss:
        ...
        # 从实际数据中获取高分辨率数据的尺寸
        # 获取一个样本来确定实际的数据尺寸
        sample_batch = next(iter(high_train_dataloader))
        y_sample, _, _, _, _ = sample_batch
        # y_sample 形状: (batch, 1, n_vars, nlat, nlon)，需要 squeeze(1) 移除时间维度
        y_sample = y_sample.squeeze(1)  # 移除时间维度，形状变为 (batch, n_vars, nlat, nlon)
        actual_nlat = y_sample.shape[2]
        actual_nlon = y_sample.shape[3]
        
        print(f"检测到高分辨率数据尺寸: nlat={actual_nlat}, nlon={actual_nlon}")
        logging.info(f"检测到高分辨率数据尺寸: nlat={actual_nlat}, nlon={actual_nlon}")
    ...
    swanlab.init(
        ...
        experiment_name=f"lr={lr}_pred_range={pred_range}_pdeweight={pde_weight}_coords_spherical_operator",
        config={
            "lr": lr,
            "betas": betas,
    ...
    model = ClimaX(default_vars=default_vars,out_dim = len(out_vars),grid_size=[jj, kk], high_gird=[jjj, kkk])
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        val_loss = evaluate(encoder, model, low_val_dataloader, high_val_dataloader, coords, device, split_name="val")
        test_loss = evaluate(encoder, model, low_test_dataloader, high_test_dataloader, coords, device, split_name="test")

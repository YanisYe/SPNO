...
class SphericalLaplacianPyramid(nn.Module):
    ...
    - 所有尺度保持 [B, C, H, W]
    """
    def __init__(self, num_scales, in_channels, use_spherical_conv=True):
        super().__init__()
        self.num_scales = num_scales
        self.use_spherical_conv = use_spherical_conv
        self.lowpass_filters = nn.ModuleList()
        
        Conv2dClass = SphereConv2d if use_spherical_conv else nn.Conv2d
...
class SphereConvBlock(nn.Module):
    def __init__(self, in_c, out_c, dropout=0.1, use_spherical_conv=True):
        super().__init__()
        ...
        # 第一个卷积：保持通道数不变
        self.conv1 = Conv2dClass(in_c, in_c, kernel_size=3, padding=1)
        # 对于1x1特征图，使用普通卷积
        self.conv1_1x1 = nn.Conv2d(in_c, in_c, kernel_size=1, padding=0)
        
        # 第二个卷积：改变通道数，bias=False（参考 ResBlock）
        self.conv2 = Conv2dClass(in_c, out_c, kernel_size=3, padding=1, bias=False)
        # 对于1x1特征图，使用普通卷积
...
class SphericalLaplacianConv(nn.Module):
    ...
    """
    def __init__(self, in_c, out_c, num_scales, dropout=0.1, share_weights=False, use_spherical_conv=True):
        super().__init__()
        self.pyramid = SphericalLaplacianPyramid(num_scales, in_c, use_spherical_conv=use_spherical_conv)

        if share_weights:
            block = SphereConvBlock(in_c, out_c, dropout, use_spherical_conv=use_spherical_conv)
            self.scale_blocks = nn.ModuleList([block for _ in range(num_scales)])
        else:
            self.scale_blocks = nn.ModuleList([
                SphereConvBlock(in_c, out_c, dropout, use_spherical_conv=use_spherical_conv)
...
class WeatherSphericalEncoder(nn.Module):
    def __init__(
        ...
        num_heads=8,
        dropout=0.1,
        use_spherical_conv=True,  # 消融实验参数：是否使用球面卷积
    ):
        super().__init__()
